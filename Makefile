...
elm-backend:
	npx elm make src/Analyser.elm --output dist/app/backend-elm.js --optimize

elm-client:
	npx elm make src/Client.elm --output dist/public/client-elm.js --optimize

elm-docs:
	npx elm make docs/Docs/Main.elm --output docs/docs.js
...
editor: tsc
	npx elm make src/Editor.elm --output dist/app/editor/elm.js
	mkdir -p dist/public
